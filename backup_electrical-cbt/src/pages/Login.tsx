...
import { getMemberByName, setCurrentUser, getCurrentExamSession, clearCurrentExamSession, saveCurrentExamSession, getMembers, initializeData, addLoginHistory, saveQuestions } from '../services/storage';
import { getAllQuestionsFromSheets } from '../services/googleSheetsService';
...
export default function Login({ onLoginSuccess, onGuestMode, onResumeExam }: LoginProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 서버 데이터 로드 실패해도 로그인은 계속 진행
      console.warn('⚠️ 서버 데이터 로드 실패 (로컬 데이터로 진행)');
    }
